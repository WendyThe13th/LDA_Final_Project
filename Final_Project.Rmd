...
smoke %>%
  ...
  geom_smooth(methed = "loess") +
  ggtitle("Figure 6: Diastolic Blood Pressure across Age")

smoke %>%
  mutate(cursmoke = as.factor(cursmoke)) %>%
  ggplot(aes(age, diabp, cursmoke, group = cursmoke, color = cursmoke)) +
  geom_smooth(method = "loess") +
  ggtitle("Figure 7: Diabetes Status across Age")
```
...
back.fit3 <- lm(sysbp ~ .-randid, data=smoke.vs)
summary(back.fit3)
#We then take out the variables with the highest p-value:
...
back.fit4 <- lm(diabp ~ .-randid, data=smoke.vs)
summary(back.fit4)
...
back.fit5 <- lm(totchol ~ .-randid, data=smoke.vs)
summary(back.fit5)
