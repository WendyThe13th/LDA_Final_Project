...
smoke %>%
  ...
  ggtitle("Figure 5: Packs a Day across Age and Sex") +
  scale_colour_hue(name = "Sex") 
```
...
step.fit = glm(cursmoke~.-randid, data = smoke.step,family = binomial)
step(step.fit, direction='backward')

#result from stepwise:
#cursmoke ~ sex + age + totchol + bmi + heartrte + educ + diabp for having the lowest AIC

#Stewise regression and backward elimination result in the same model.
```
...
###stepwise regression###
step.fit2 = lm(cigpday~.-randid, data = smoke.vs2)
