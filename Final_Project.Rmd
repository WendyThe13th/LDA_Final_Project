...
smoke %>%
  ...
  geom_smooth(method = "loess", se = F) +
  ggtitle("Figure 7: Totchol across Age")
```
...
```{r}
glm1 <- glm(cursmoke ~ age+sex, data=smoke_vs1,family = 'binomial')
glm2 <- glm(cursmoke ~ age+sex+bpmeds, data=smoke_vs1,family = 'binomial')
a=(summary(glm1)$coefficients[2])-(1.96 * (summary(glm1)$coefficients[5])) 
b= (summary(glm1)$coefficients[2])+(1.96 * (summary(glm1)$coefficients[5]))
c=summary(glm2)$coefficients[2]
exp(a)
...
!(exp(f)>=exp(d) & exp(f)<=exp(e))
```
bpmdes is not confounder for sex and age with cursmoke
...
smoke_vs5 = smoke %>% 
  ...
  na.omit()
lmer_3 <- lmer(sysbp ~ cursmoke + sex + bmi + age + (1|randid), data = smoke_vs5)
summary(lmer_3)
```
...
smoke_vs6 = smoke %>% 
  ...
  mutate(sex=as.factor(sex),cursmoke=as.factor(cursmoke)) %>% 
  na.omit()
lmer_4 <- lmer(diabp ~ cursmoke + sex + bmi + age + (1|randid), data = smoke_vs6)
summary(lmer_4)
```
...
smoke_vs7 = smoke %>% 
  ...
  mutate(sex=as.factor(sex),cursmoke=as.factor(cursmoke)) %>% 
  na.omit()
lmer_5 <- lmer(totchol ~ cursmoke + sex + bmi + age + (1|randid), data = smoke_vs7)
summary(lmer_5)
```
