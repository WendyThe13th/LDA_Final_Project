...
smoke %>%
  ...
  geom_smooth(method = "loess") +
  ggtitle("Figure 4: Cigarette per Day across Age")
...
By rule of thumb, bmi is confounder. Based on the literature view, age and sex can be potential confounders as they can affect smoke status and sysbp at the same time. So we still put them into model.
...
```{r}
#cursmoke
2*(pnorm(-0.008418/0.432847))
```
Not significant,but include because this is our interest
```{r}
#cursmoke 95% CI 
...
pvalue<-round(2*(1-pnorm(abs(coefs4$t.value))),3)
pvalue
```
...
```{r}
#sex
2*(pnorm(-0.53287/0.29288))
```
although not significant, it is confounder, so we still put in into the model
```{r}
#CI sex
